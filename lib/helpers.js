...
// Histórico dos últimos `n` meses terminando em `key` (mais antigo primeiro).
// Cada item: { key, label, receita, despesa, saldo }.
export const computeHistory = (data, key, n = 6) => {
  const out = []
  for (let i = n - 1; i >= 0; i--) {
    const k = shiftMonth(key, -i)
    const s = computeMonthSummary(data, k)
    const m = parseInt(k.split('-')[1], 10) - 1
    out.push({ key: k, label: MONTHS_SHORT[m], receita: s.totalIncome, despesa: s.totalExpenses, saldo: s.balance })
  }
  return out
}

// Dívidas = itens parcelados (despesas fixas + despesas variáveis), com progresso
// derivado de numParcelas + mês de início, calculado para o mês visualizado.
export const deriveDebts = (data, viewedMonth) => {
  const items = []
  for (const r of data.recurring) {
    if (r.parcelado && r.numParcelas > 0) items.push(r)
  }
  const seen = new Set()
  for (const md of Object.values(data.monthlyData || {})) {
    for (const e of md.expenses || []) {
      if (e.parcelado && e.numParcelas > 0 && !seen.has(e.id)) { seen.add(e.id); items.push(e) }
    }
  }
  const [vy, vm] = viewedMonth.split('-').map(Number)
  return items
    .map((it) => {
      const startKey = it.startMonth || (it.date ? it.date.slice(0, 7) : null)
      if (!startKey) return null
      const [sy, sm] = startKey.split('-').map(Number)
      const elapsed = (vy - sy) * 12 + (vm - sm)
      const parcelasPagas = Math.max(0, Math.min(it.numParcelas, elapsed))
      const total = it.amount * it.numParcelas
      const pago = it.amount * parcelasPagas
      const cat = findCategory(it.category)
      return {
        id: it.id,
        nome: it.description,
        cor: cat?.color || '#8a8378',
        amount: it.amount,
        total,
        pago,
        restante: total - pago,
        parcelasTotal: it.numParcelas,
        parcelasPagas,
        dueDay: it.dueDay || null,
        quitada: parcelasPagas >= it.numParcelas,
        startKey,
      }
    })
    .filter(Boolean)
    .sort((a, b) => Number(a.quitada) - Number(b.quitada) || b.restante - a.restante)
}

// Eventos do calendário do mês visualizado: despesas fixas (por dia de vencimento),
// despesas variáveis e ganhos extras (pelo dia da data). Receita principal não tem
// dia definido, então fica de fora do calendário (placeholder na UI).
export const derivePlanning = (data, viewedMonth) => {
  const [y, m] = viewedMonth.split('-').map(Number)
  const daysInMonth = new Date(y, m, 0).getDate()
  const firstDow = new Date(y, m - 1, 1).getDay() // 0=Dom

  const disabledIds = data.disabledRecurring[viewedMonth] || []
  const activeRecurring = data.recurring.filter((r) => !disabledIds.includes(r.id))
  const md = data.monthlyData[viewedMonth] || { expenses: [], extras: [] }

  const events = []
  for (const r of activeRecurring) {
    if (r.dueDay) {
      const cat = findCategory(r.category)
      events.push({ dia: Math.min(r.dueDay, daysInMonth), desc: r.description, tipo: 'despesa', valor: r.amount, cat: cat?.label || 'Outros', fixa: true })
    }
  }
  for (const e of md.expenses) {
    const cat = findCategory(e.category)
    events.push({ dia: new Date(e.date + 'T12:00:00').getDate(), desc: e.description, tipo: 'despesa', valor: e.amount, cat: cat?.label || 'Outros' })
  }
  for (const e of md.extras) {
    events.push({ dia: new Date(e.date + 'T12:00:00').getDate(), desc: e.description, tipo: 'receita', valor: e.amount, cat: 'Ganho extra' })
  }
  events.sort((a, b) => a.dia - b.dia)
  return { daysInMonth, firstDow, events }
}
