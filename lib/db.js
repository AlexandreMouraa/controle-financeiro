...
export async function loadAllData(userId) {
  const [
    ...
    supabase.from('paid_recurring').select('*').eq('user_id', userId),
  ])
  ...
  for (const row of paidRows || []) {
    if (!paidRecurring[row.month]) paidRecurring[row.month] = []
    paidRecurring[row.month].push(row.recurring_id)
  }

  const goalRow = (goalRows || [])[0]
  const goal = goalRow
    ? { totalAmount: Number(goalRow.total_amount), startMonth: goalRow.start_month, months: goalRow.months, initialAmount: Number(goalRow.initial_amount) || 0 }
    : null
...
// --- Paid state (checklist "pago") ---

// Despesa avulsa: "pago" é por linha (a linha já pertence a um mês)
export async function setExpensePaid(userId, id, paid) {
  return supabase.from('expenses').update({ paid }).eq('id', id).eq('user_id', userId)
}

// Despesa fixa: "pago" é por mês (a mesma fixa se repete todo mês)
export async function markRecurringPaid(userId, month, recurringId) {
  return supabase.from('paid_recurring').insert({ user_id: userId, month, recurring_id: recurringId })
}

export async function unmarkRecurringPaid(userId, month, recurringId) {
  return supabase
    .from('paid_recurring')
    .delete()
    .eq('user_id', userId)
...
export async function upsertGoal(userId, { totalAmount, startMonth, months, initialAmount = 0 }) {
  return supabase
    .from('goals')
    .upsert(
      { user_id: userId, total_amount: totalAmount, start_month: startMonth, months, initial_amount: initialAmount },
      { onConflict: 'user_id' }
    )
}
...
export async function replaceAllData(userId, data) {
  // disabled_recurring e paid_recurring têm FK para recurring, então deleta primeiro
  await supabase.from('disabled_recurring').delete().eq('user_id', userId)
  await supabase.from('paid_recurring').delete().eq('user_id', userId)
  ...
  if (data.goal) {
    inserts.push(supabase.from('goals').insert({
      ...
      initial_amount: data.goal.initialAmount || 0,
    }))
  }
  ...
  const paidEntries = []
  for (const [month, ids] of Object.entries(data.paidRecurring || {})) {
    for (const oldId of ids) {
      const newId = idMap[String(oldId)]
      if (newId) paidEntries.push({ user_id: userId, month, recurring_id: newId })
    }
  }
  if (paidEntries.length > 0) inserts.push(supabase.from('paid_recurring').insert(paidEntries))
